...
**Download and extract to `data/` directory before running:**
```bash
mkdir data
# Place extracted .wav files into data/
```
...
```bash
# Update the data path in humming_classifier_improved.py (line 540)
# Then run:
...
| MFCC | 78 | 13 MFCC + delta + delta2 + statistics |
